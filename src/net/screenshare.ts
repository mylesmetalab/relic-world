...
function makeVideo(stream: MediaStream): HTMLVideoElement {
  ...
  video.autoplay = true;
  video.muted = true; // no audio track on this stream anyway; keeps autoplay policies happy
...
export class ScreenShare {
  ...
  /** Your own outgoing share, so you see what you're sharing too — same as
   *  Zoom's own-preview thumbnail while presenting. Keyed separately from
   *  `incoming` (which is strictly "from a peer") but folded into `list()`
   *  under `net.selfId` so the HUD can treat it identically to a peer's tile. */
  private local: HTMLVideoElement | null = null;
  private readonly incoming = new Map<string, SharedScreen>();
  onChange: ((enabled: boolean, error?: string) => void) | null = null;
  /** Fires whenever a peer starts/stops sharing (or you do), so the HUD can
   *  rebuild its tiles. */
   ...
   *  your own outgoing share first (keyed under `net.selfId`), then peers'. */
  list(): SharedScreen[] {
    const mine = this.local ? [{ peerId: this.net.selfId, video: this.local }] : [];
    return [...mine, ...this.incoming.values()];
  }
}
