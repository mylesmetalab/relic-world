...
export type Biome = {
  ...
  ceilLift: number;
  /** Overrides `CFG.world.climbSolidity` for this biome's wall-climb grip
   *  check (lower = fewer grippable surfaces, reads as slippery; higher =
   *  more forgiving, reads as soft/crumbly). Omit to use the global default. */
...
export const BIOMES: Biome[] = [
  ...
  // Slippery: fewer grippable surfaces than the global default (0.85).
  ...
  // Soft/crumbly: more forgiving than the global default (0.85).
  ...
  // Warm dusk-lit canyon: reuses the unused "Dusk Ridge" colorway, open and
  // gently rolling, moderate stalagmite share.
  { name: "Dusk Ridge", ramp: ramp("Dusk Ridge"), terrace: 0.5, relief: 1.3, rocks: 10, tallShare: 0.35, ceilLift: 0,
    pen: { hatchRange: 0.58, black: 0.28, pitchScale: 1.2, nib: 0.85, cracks: 0.35, stipple: 0.3, hatchRot: 0.6, formFollow: 0.4 } },
  // Cramped and tangled: reuses the unused "Ash Field" colorway, a low
  // squeezed ceiling, dense small rubble, heavy cracks/stipple.
  { name: "Root Cellar", ramp: ramp("Ash Field"), terrace: 0, relief: 0.5, rocks: 22, tallShare: 0.1, ceilLift: -3,
    pen: { hatchRange: 0.6, black: 0.35, pitchScale: 1.4, nib: 1.1, cracks: 0.75, stipple: 0.65, hatchRot: -0.6, formFollow: 0.45 } },
  // Bright and glassy: sparse tall spires, a new palette ramp, and the
  // smoothest grip of any biome — the glass reads as slippery underfoot.
  { name: "Crystal Vein", ramp: ramp("Crystal Vein"), terrace: 1.0, relief: 0.9, rocks: 5, tallShare: 0.7, ceilLift: 0,
    climbSolidity: 0.55,
    pen: { hatchRange: 0.4, black: 0.15, pitchScale: 0.7, nib: 1.8, cracks: 0.05, stipple: 0.05, hatchRot: 0.1, formFollow: 0.15 } },
];
